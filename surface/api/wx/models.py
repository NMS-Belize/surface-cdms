...
class Variable(BaseModel):
    ...
    range_min = models.FloatField(null=True, blank=True, verbose_name = 'Global threshold (Manual) Range Min')
    range_max = models.FloatField(null=True, blank=True, verbose_name = 'Global threshold (Manual) Range Max')

    range_min_hourly = models.FloatField(null=True, blank=True, verbose_name = 'Global threshold (Automatic) Range Min')
    range_max_hourly = models.FloatField(null=True, blank=True, verbose_name = 'Global threshold (Automatic) Range Max')

    step = models.FloatField(null=True, blank=True, verbose_name = 'Global threshold (Manual) Step')
...
class Station(BaseModel):    
    ...
    is_reference = models.BooleanField(default=False, verbose_name="Reference Station")
...
class QcRangeThreshold(BaseModel):
    ...
    def __str__(self):
        return f"station={self.station.code} variable={self.variable.symbol} month={self.month}"

    class Meta:
        ordering = ('station', 'variable', 'month')
...
class QcPersistThreshold(BaseModel):
    ...
    class Meta:
        ordering = ('station', 'variable', 'window')
        unique_together = ("station", "variable")
...
class StationFileIngestion(BaseModel):
    ...
    process_in_chunks = models.BooleanField(default=False)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=~(
                    Q(is_highfrequency_data=True)
                    & Q(process_in_chunks=True)
                ),
                name="station_file_ingestion_not_hf_and_chunked",
            ),
        ]
    ...
    def clean(self):
        ...
        if self.is_highfrequency_data and self.process_in_chunks:
            raise ValidationError(
                "A station file ingestion cannot be both high-frequency data and process-in-chunks data."
            )
...
class StationDataFile(BaseModel):
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                ...
                name="station_data_file_not_hf_and_chunked",
            ),
        ]
    ...
    def clean(self):
        ...
        if self.is_highfrequency_data and self.process_in_chunks:
            raise ValidationError(
                "A station data file cannot be both high-frequency data and process-in-chunks data."
            )
...
class ManualStationDataFile(BaseModel):
    ...
    override_data_on_conflict = models.BooleanField(default=False)
    is_manually_validated = models.BooleanField(default=False)
...
class MaintenanceReportEquipment(BaseModel):
    ...
    equipment_type = models.ForeignKey(EquipmentType, on_delete=models.DO_NOTHING)
    equipment_order = models.CharField(max_length=1, choices=EquipmentOrder.choices, default=EquipmentOrder.PRIMARY_EQUIPMENT)
    old_equipment = models.ForeignKey(Equipment, on_delete=models.PROTECT, related_name='old_equipment', null=True)
    new_equipment = models.ForeignKey(Equipment, on_delete=models.PROTECT, related_name='new_equipment', null=True)
...
class WxPermissionCategory(BaseModel):
    """
    A bucket for permission pages (used for UI grouping).
    Examples: Stations, Data Capture, Quality Control, WIS2, Maintenance, Admin.
    """
    name = models.CharField(max_length=64, unique=True)
...
class WxPermissionPages(BaseModel):
    """
    A single "page" in the system that can have read/write/delete access.
    Identified by url_name (the Django urlpattern 'name=...').
    """
    category = models.ForeignKey(
        WxPermissionCategory,
        on_delete=models.PROTECT,   # PROTECT prevents deleting a category used by pages
        related_name="pages",
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                ...
                name="unique_stagedrawdata_file_datetime_station_variable",
            ),
        ]
        indexes = [
            models.Index(fields=["station_data_file", "station", "variable"]),
            models.Index(fields=["station", "variable", "datetime"]),
        ]


class StagedRawDataTask(BaseModel):
    station_data_file = models.ForeignKey(
        StationDataFile,
        on_delete=models.CASCADE,
    )
